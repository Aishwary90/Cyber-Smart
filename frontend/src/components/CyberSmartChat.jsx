...
function isExplanationIntent(message) {
  return /\b(why|how|explain|reason|result|classification|verdict|not crime)\b/i.test(message);
}
...
function buildCrimeIntroResponse(classification, incidentText) {
  ...
  return {
    text: firstQuestion
      ? `Based on what you've described, this looks like **${crimeLabel}**. To continue, I need one detail: **${firstQuestion.question}**`
      : `Based on what you've described, this looks like **${crimeLabel}**.`,
    classification: {
      type: "CRIME",
    ...
    reasoning: buildClassificationReasoning(classification, incidentText),
    suggestions: firstQuestion?.options || ["What should I do now?", "How did you decide this?"],
  };
}
...
function buildNotCrimeResponse(classification, incidentText) {
  ...
  return {
    text:
      `This currently looks more like **${notCrimeName}** than a cybercrime. ` +
...
function buildUnclearResponse(classification, incidentText) {
  return {
    text:
      "I need a little more detail before I can classify this properly. Tell me whether this involved money loss, account access, or a suspicious message or link.",
    classification: {
      type:
...
function buildFinalVerdictResponse(verdictResponse, fallbackClassification) {
  ...
  return {
    text: isCrime
      ? `Based on your answers, this now looks like **${verdict.subtitle || fallbackClassification?.crimeType || "a cybercrime"}**. Start with: ${(verdict.immediate_actions || []).slice(0, 2).join(" | ") || "preserve evidence and report quickly"}.`
    ...
    reasoning: verdict.decision_reasoning || null,
    suggestions: isCrime
...
const CyberSmartChat = ({ currentUser, onNewCase, onError }) => {
  ...
  async function runStatefulFallback(message) {
    if (caseState?.pendingQuestion) {
      if (isExplanationIntent(message)) {
        const classification = caseState.classification;
        return buildAssistantMessage({
          text:
            `${buildClassificationReasoning(classification, caseState.incidentText).summary} ` +
          ...
          reasoning: buildClassificationReasoning(classification, caseState.incidentText),
      ...
      if (!nextQuestionResponse.done && nextQuestionResponse.question_id) {
        ...
        return buildAssistantMessage({
          ...
          reasoning: buildClassificationReasoning(caseState.classification, caseState.incidentText),
    ...
    if (caseState && isExplanationIntent(message)) {
      ...
      return buildAssistantMessage({
        text: verdictReasoning?.summary || "This is the current decision summary for your case.",
        classification: {
          type:
        ...
        suggestions: caseState.pendingQuestion?.options || ["Start a new case"],
      });
    }
